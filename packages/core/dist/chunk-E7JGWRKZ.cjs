...
function bootstrapMiddleware(config = {}) {
  return async (c, next) => {
    ...
    try {
      ...
      const migrationService = new chunkRFXM6F34_cjs.MigrationService(c.env.DB);
      await migrationService.runPendingMigrations();
      console.log("[Bootstrap] Syncing collection configurations...");
      try {
        await chunkQCIJ7TJV_cjs.syncCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing collections:", error);
      }
      console.log("[Bootstrap] Syncing form collections...");
      try {
        await chunkQCIJ7TJV_cjs.syncAllFormCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing form collections:", error);
      }
      if (!config.plugins?.disableAll) {
        console.log("[Bootstrap] Bootstrapping core plugins...");
        const bootstrapService = new chunkQCIJ7TJV_cjs.PluginBootstrapService(c.env.DB);
...
//# sourceMappingURL=chunk-E7JGWRKZ.cjs.map
//# sourceMappingURL=chunk-E7JGWRKZ.cjs.map